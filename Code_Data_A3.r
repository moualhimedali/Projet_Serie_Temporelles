...
Data_A3= read.table(file = file.choose(),header = F)
attach(Data_A3)
Data_A3
#Type de data
class(Data_A3)
#Convertir DataFrame en Serie temporelle
Data_A3.ts=ts(data = Data_A3, start = c(1992,1),end = c(2007,12) ,frequency =12 )
Data_A3.ts
#Effectuer un analyse a long terme
#Tandance , saisonalite , aleatoire
class(Data_A3.ts)
print(Data_A3.ts)
plot(Data_A3.ts,xlab="Date")
plot.ts(Data_A3.ts)
#interpretation_1:
#La série d'origine figure dans le graphique du Data_A3.ts comporte une saisonnalité marquée avec une variabilité croissante au cours du temps.
#On constate que son niveau moyen s’accroıt avec le temps, de manière quasi-exponentielle ou polynomiale : on traduit encore cette évolution sous le terme de tendance.
#=>Il semble y avoir une saisonnalité mais également une tendance.
#remove(Data_A3)
#remove(Data_A3.ts)
tsp(Data_A3.ts)
plot(Data_A3.ts,plot.type = "multiple",lty = 50:300)
cycle(Data_A3.ts)
layout(1:1)
plot(aggregate(Data_A3.ts))
boxplot(Data_A3.ts~cycle(Data_A3.ts))
...
#Auto-corrélation
lag.plot(Data_A3.ts,lags=12)
acf(Data_A3.ts, lag.max = 12, type = c("correlation"), plot = FALSE)
#D'apres ACF on remarque que la serie data_A3.ts admet une tandance decoroissante.
#effectuer une analyse a court terrme:
modeleAditiveData_A3 = decompose(Data_A3.ts , "additive")
modeleMultiData_A3 = decompose(Data_A3.ts , "multiplicative")
plot(modeleAditiveData_A3)
plot(modeleMultiData_A3)
...
#Le corrélogramme
acf(modeleAditiveData_A3$random , na.action = na.pass)
acf(modeleMultiData_A3$random , na.action = na.pass)
#la deuxiéme modele est (admet set batons qui depassent l'intervalle ) on choisie modele additive
#tous les informations ont ete bien exploite
#danc on peut passé au ajustement
#interpretation: le coefficient pour le décalage 0.0833( càd 1/12) vaut 0.916.
...
#pour k = 4
moyenneMobile4 = filter(Data_A3.ts,rep(1/4,4),method = "convolution")
plot(Data_A3.ts)
lines(moyenneMobile4 , col="red")
#pour k = 5
moyenneMobile5 = filter(Data_A3.ts,rep(1/5,5),method = "convolution")
plot(Data_A3.ts)
lines(moyenneMobile5 , col="red")
...
tps <-1 : length(Data_A3.ts)
MC<-matrix(0, length(Data_A3.ts), 6)
MS<-matrix(0, length(Data_A3.ts), 6)
for (i in 1:6) MC[ ,i]<-cos(2 * pi * tps * i/12)
for (i in 1:6) MS[ ,i]<-sin(2 * pi * tps* i/12)
mod3<-lm(Data_A3.ts~tps + MC + MS)
summary(mod3)
mod4<- lm(Data_A3.ts~tps + I(t^2) + I(t^3) + I(t^4) + I(t^5) + MC + MS)
summary(mod4)
AIC(mod4,mod3,model2,model1)
...
#notre p_value = 0.0008591 < 5% donc notre processus est stationnaire et par 
